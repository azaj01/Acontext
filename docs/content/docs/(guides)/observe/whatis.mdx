...
2. **Messages buffer** — Acontext batches messages before processing. The buffer flushes when it reaches a configured turn count (`buffer_max_turns`) or when no new messages arrive for a configured idle period (`buffer_ttl_seconds`). You can also call `flush()` to trigger processing immediately.
